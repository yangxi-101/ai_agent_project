...
> 依赖清单见 `requirements_full.txt`。注意：它是**通用环境的全量快照**（211 行，含 pandas/opencv/onnx 等本项目未使用的包），不是最小依赖，详见桌面上的 `min_RAG_AGENT_升级指南/UPGRADE_GUIDE.md` 的 v0.0.2 节。
...
### 10.2 但召回质量不成立 —— 分数与语义无关

查询"下单后什么时候发货"（正确答案是第 2 条「下单后多久可以发货？」），实测 top-5：

| 排名 | 分数 | 内容 | 是否正确 |
| --- | --- | --- | --- |
| 1 | 0.4318 | 4. 退货政策是怎样的？ | ✗ |
| 2 | 0.4195 | 8. 账号忘记密码怎么办？ | ✗ |
| 3 | 0.4052 | 5. 如何查询我的物流信息？ | 勉强相关 |
| **4** | **0.3878** | **2. 下单后多久可以发货？** | **✓ 正确答案排第 4** |
| 5 | 0.3809 | 1. 你们支持哪些支付方式？ | ✗ |
...
| 下单后什么时候发货 | 0.4304 | 相关 |
| 帮我写一首诗 | 0.4415 | 完全无关 |
| **今天天气怎么样** | **0.5162** | **完全无关，分数却最高** |

「4. 退货政策是怎样的？」在**所有**查询下都排第 1——包括"今天天气怎么样"和"帮我写一首诗"。它是分数磁铁，排序没有语义基础。

### 10.3 根因：语料同质化 + 嵌入模型区分不开

10 条切片彼此的两两余弦相似度：

- **均值 0.6776，最小 0.5427，最大 0.7976**
- 最像的一对：`6. 客服工作时间是什么时候？` ↔ `9. 商品有质量问题怎么办？` = **0.7976**（语义上毫无关系）
- 对照基线：`猫咪喜欢吃什么` ↔ `股票市场今天大跌` = **0.3765**

也就是说，**这份语料里任意两条的相似度（0.54~0.80）都远高于"完全无关"的基线（0.3765）**。它们在这个嵌入空间里几乎互为近义，模型无从区分。原因指向语料本身：10 条都是"XX 怎么办？"式的短客服文本，句式、词汇高度重叠。

已排除的可能：
- 不是向量过期 —— 新旧向量一致度 0.9997+
- 不是 Milvus 算错 —— 本地重算排名完全一致
- 不是模型完全没判别力 —— `猫咪喜欢吃什么`↔`小猫平时吃什么食物` = 0.5542 vs 无关 0.3765，模型本身有区分能力
- 加 Qwen3-Embedding 推荐的 `Instruct: ...\nQuery:` 前缀 —— **无效**，排名不变（top-1 仍是退货政策）

### 10.4 这个结果推翻了一条既有建议

README 与升级指南原先都把"加相关性阈值"列为 v0.1.0 的首要改造。**实测数据不支持这个做法**：阈值的前提是"相关分数 > 无关分数"，而实测是**反的**（相关 0.4304 < 无关 0.5162）。卡在中间会把正确答案一起滤掉，卡在高位则无关问题照样进。

阈值只能等召回变得可分之后再加。**当前第一优先级是让召回恢复区分能力**，改动方向见升级指南 v0.1.0（已按实测修正）。

### 10.5 一个必须点明的假象

`python agent.py` 返回的答案是**正确**的：

> 一般情况下，订单会在支付成功后24小时内发货。大促期间（如双11、618）可能略有延迟，请关注页面提示。

但这**不是检索的功劳**——答案来自排名第 4 的片段，是对话模型自己在 5 条上下文里挑出了正确的那一条。**检索失效被模型的二次筛选掩盖了。**

这正是这个项目最危险的地方：它看起来能用，测起来也能过，但底下那层已经坏了。只要换成 1000 条知识库、或者用户问一个知识库外的问题，掩盖就会失效。

---

## 10. 下一步

完整的**升级路线图**（v0.0.2 → v0.4.0，含每一步的改动文件和验收标准）以及**从零构建顺序**，已单独存放在桌面：

```
C:\Users\10144\Desktop\min_RAG_AGENT_升级指南\UPGRADE_GUIDE.md
```
